...
def arc_exchange(routes, points, demands, Q,mode):
    ...
    if mode == 'feasible' :
        ...
        # feasible = inst.constraints(candidate_routes, demands, Q)
...
def swap_intra_route(routes, points, demands, Q, mode):
    ...
    if mode == 'feasible':
        ...
        nodes = np.random.choice(r0[1,-1],size=2,replace=False)
        node1 = int(nodes[0])
        node2 = int(nodes[1])
...
def two_opt_exchange_outer(routes, points, demands, Q, mode):
    ...
    if mode == 'feasible':
        ...
        difference = - dist(points[r0]) - dist(points[r1])
        + dist(points[r0_new]) + dist(points[r1_new])
...
def neighbour(case,routes, points, demands, Q, mode): #mode può assumere i valori 'feasible' oppure 'exploration'
    ...
    if case == 0:
        new_routes, difference = arc_exchange(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 0 no improvement \n")
    elif case == 1:
        new_routes, difference = swap_inter_route(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 1 no improvement \n")
    elif case == 2:
        new_routes, difference = move_node(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 2 no improvement \n")
    elif case == 3:
        new_routes, difference = swap_intra_route(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 3 no improvement \n")
    elif case == 4:
        new_routes, difference = two_opt_exchange_outer(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 4 no improvement \n")
    elif case == 5:
        new_routes, difference = two_opt_exchange_inner(routes, points, demands, Q, mode)
        if difference >= 0 and debug == 'activated':
            print("\n 5 no improvement \n")
